...
export default function OrcamentoAprovadoFilter() {
  ...
  const orcamentosAprovadoList = useSelector((state: RootState) => state.orcamentosAprovadoList);
  ...
  useEffect(() => {
    ...
    dispatch(fecthOrcamentosAprovados({ 
  ...
  return (
    <StyledView className="pb-4">
      <StyledView className="w-full flex-row justify-between items-center">
        ...
        <StyledPressable
          ...
          onPress={() => dispatch(fecthOrcamentosAprovados({url: `${queryParams.toString()}`}))}
        >
    ...
    <StyledView className="py-2 flex flex-row justify-center items-center gap-x-4">
        ...
        <StyledPressable className="text-lg text-white  text-center" onPress={() => setYear((year) => year + 1)}>
            <StyledText className="text-lg text-white  text-center">
                +
            </StyledText>
        </StyledPressable>
    </StyledView>
    <StyledView className={"flex flex-row  gap-x-3 gap-y-2 flex-wrap justify-center items-center"}>
      {
        months.map((item: string, index: number) => {
          return(
            <StyledPressable key={index} onPress={() => setMonth(() => {
                if(index + 1 === month){
                    return undefined
                }
                ...
                ${index + 1 ==  month ? "bg-gray-800 color-red font-semibold border-[1px] border-white" : "bg-gray-400"} 
                rounded-sm cursor-pointer h-7 w-12  flex justify-center items-center`}
            >
              <StyledText className={`${index + 1 ==  month ? "text-white font-semibold" : "bg-gray-400"} `}>{item}</StyledText>
            </StyledPressable>
          )
        })
      }
    ...
    <StyledView className={"py-2"}>
        <StyledText className="text-white font-semibold">{orcamentosAprovadoList.orcamentosAprovado.length} orcamentos</StyledText>
    </StyledView>
    <BudgetModal
