...
import { GuestItemList } from "./guest-item-list";
import { GuestType } from "@store/guest/guest-slice";
...
export const StyledFlatList = styled(FlatList<GuestType>);
...
export function GuestList() {
  ...
  const guestList  : GuestType[] = useSelector((state: RootState) => state.guestState.guestList);
  const proposal  : ProposalType = useSelector((state: RootState) => state.proposalList.proposal);

  const renderItem = useCallback(({ item }: { item: GuestType }) => {
    return <GuestItemList guest={item} />;
  }, []);
    ...
    <StyledView className="w-full flex-1">
      <StyledView className="flex flex-row justify-between items-center pb-3">
        ...
        <StyledView className="flex flex-row  justify-start items-center gap-x-2">
          ...
          <StyledText  className="text-custom-white font-light">
            {`${guestList?.filter((item: GuestType) => item.attendance === true).length}`}
          </StyledText>
        </StyledView>
      </StyledView>
      <StyledFlatList
        ...
        keyExtractor={(item: GuestType) => item?.id} // Garante que a chave é única
