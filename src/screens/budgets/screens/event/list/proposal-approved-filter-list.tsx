...
export function ApprovedProposalFilter() {
  const [query, setQuery] = useState("");
  const [year, setYear] = useState<any>(new Date().getFullYear());
  const [month, setMonth] = useState<number>(new Date().getMonth() + 1);

  const dispatch = useDispatch();
  const approvedproposals = useSelector(
    (state: RootState) => state.proposalList.approvedproposals
  );

  const venue = useSelector((state: RootState) => state.venueList);
    ...
    <StyledView className="pb-4">
      ...
      <StyledView className={"py-2"}>
        <StyledText className="text-white font-semibold">
          {approvedproposals.length} orcamentos
        </StyledText>
      </StyledView>
    </StyledView>
  );
